    ...
    <div class="col-md-7">
      <div class="mt-3 mb-3">
        <button class="btn btn-primary w-100" id="btn-create">
          <i class="icon fas fa-plus pr-1"></i>Prodi</button>
      </div>

    <div class="card shadow mb-4">
      <div class="card-header py-3">
          <h6 class="mt-2 font-weight-bold text-primary">Data Prodi</h6>
      </div>
      <div class="card-body">
...
<div class="modal fade" id="modal-edit" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <input type="hidden" id="admin_prodis_id">
          <div class="form-group">
  ...
  <script>
    $('body').on('click', '#btn-edit', function () {
      let admin_prodis_id = $(this).data('id');
        $.ajax({
          url: `{{ url('admin/prodi/show/${admin_prodis_id}') }}`,
          type: "GET",
          cache: false,
          success: function (response) {
            // console.log(response.data);
            $('#admin_prodis_id').val(response.data.id);
    ...
    $('#update').click(function (e) { 
        e.preventDefault();
        let admin_prodis_id = $('#admin_prodis_id').val();
        let name   = $('#name_edit').val();
        let token   = $("meta[name='csrf-token']").attr("content");
  
        $.ajax({
            url: `{{ url('admin/prodi/update/${admin_prodis_id}') }}`,
            type: "PUT",
            cache: false,
            data: {
                'name': name,
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        $.ajax({
          url: `{{ url('/admin/prodi/destroy/${admin_prodis_id}') }}`,
          type: "DELETE",
          cache: false,
          data: {
            "_token": token
