    ...
    <div class="col-md-7">
      <div class="mt-3 mb-3">
        <button class="btn btn-primary w-100" id="btn-create">
          <i class="icon fas fa-plus pr-1"></i>Role</button>
      </div>

    <div class="card shadow mb-4">
      <div class="card-header py-3">
          <h6 class="mt-2 font-weight-bold text-primary">Data Role</h6>
      </div>
      <div class="card-body">
  ...
  <script>
    ...
    $(document).ready(function() {
      $('#dataTable').DataTable({
        ...
        serverSide : true,
        ajax : {
          url: "{{ route('admin/role.get_datatable') }}",
        //   type: 'GET'
        },
        columns: [
          {data:'name',name:'name'},
...
<div class="modal fade" id="modal-create" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <script>
    ...
    $('#store').click(function (e) { 
        ...
        $.ajax({
            url: `{{ route('admin/role.post_store') }}`,
            type: "POST",
            cache: false,
            data: {
                'name': name,
...
<div class="modal fade" id="modal-edit" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <input type="hidden" id="admin_roles_id">
          <div class="form-group">
  ...
  <script>
    $('body').on('click', '#btn-edit', function () {
      let admin_roles_id = $(this).data('id');
        $.ajax({
          url: `{{ url('admin/role/show/${admin_roles_id}') }}`,
          type: "GET",
          cache: false,
          success: function (response) {
            // console.log(response.data);
            $('#admin_roles_id').val(response.data.id);
    ...
    $('#update').click(function (e) { 
        e.preventDefault();
        let admin_roles_id = $('#admin_roles_id').val();
        let name   = $('#name_edit').val();
        let token   = $("meta[name='csrf-token']").attr("content");
  
        $.ajax({
            url: `{{ url('admin/role/update/${admin_roles_id}') }}`,
            type: "POST",
            cache: false,
            data: {
  ...
  <script>
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        $.ajax({
          url: `{{ url('/admin/role/destroy/${admin_roles_id}') }}`,
          type: "DELETE",
          cache: false,
          data: {
            "_token": token
